...
dataHandler.prototype = {
    checkPlayerData: function(player) {
        ...
        TitlesData.PlayerData[steamID].isAdmin = TitlesData.PlayerData[steamID].isAdmin || this.hasPermission(player, this.Config.Permissions.staff) || false;
...
function setter(player) {
    var steamID = rust.UserIDFromPlayer(player);
    if (TitlesData.PlayerData[steamID].isAdmin && this.Config.Settings.noAdmin) return false;
    var oldRank = TitlesData.PlayerData[steamID].Rank;
    var i = 0,
        j = this.Config.main.length,
        kills = TitlesData.PlayerData[steamID].Kills,
        karma = TitlesData.PlayerData[steamID].Karma,
        oldRank = TitlesData.PlayerData[steamID].Rank;

    this.setRank(steamID)
    if (this.Config.Settings.useBoth) this.setPrefix(steamID);
}

setter.prototype = {
    setRank: function(steamID) {
        var this.helper = new helpers();
        for (i; i < j; i++) {
            if (karmaOn && this.helper.getClosest([], karma) === this.Config.main[i].karma) {
                TitlesData.PlayerData[steamID].Title = this.Config.main[i].title;
                TitlesData.PlayerData[steamID].Rank = this.Config.main[i].rank;
            } else if (!karmaOn && this.helper.getClosest([], kills) === this.Config.main[i].killsNeeded) {
                TitlesData.PlayerData[steamID].Title = this.Config.main[i].title;
                TitlesData.PlayerData[steamID].Rank = this.Config.main[i].rank;
            }
        }
    ...
    setPrefix: function(steamID) {
        for (var t = 0; t < this.Config.prefixTitles.length; t++) {
            if (TitlesData.PlayerData[steamID].Prefix === "" && this.Config.prefixTitles[t].default) {
                TitlesData.PlayerData[steamID].Prefix = this.Config.prefixTitles[t].title;
            }

        }
    },
    autoSet: function(steamID) {
        //TODO: build system to auto set prefixes based on permissions
    }
};
...
function deathHandler(options) {
    this.defaults = {}
    this.helper = new helpers();
}

deathHandler.prototype = {
    init: function(entity, hitinfo) {
        this.victim = entity;
        this.attacker = hitinfo.Initiator;

        if (!this.attacker.ToPlayer() || !this.victim.ToPlayer() || this.attacker === null) return false;
        if (this.Config.Settings.antiSleeper && this.deathObj.victim.IsSleeping()) return false;

        this.victimID = rust.UserIDFromPlayer(this.victim);
        this.attackerID = rust.UserIDFromPlayer(this.attacker);
        this.deathObj = {
            karmaOn: this.Config.Settings.karma,
            victimKarma: TitlesData.PlayerData[this.victimID].Karma,
            attackerKarma: TitlesData.PlayerData[this.attackerID].Karma
        };

        if (this.attacker.displayName !== this.victim.displayName) {
            this.handleDeaths();
        } else {
            this.handleSuicide();
        }
    ...
    handleDeaths: function() {
        if (this.deathObj.karmaOn && this.deathObj.victimKarma >= 0) {
            this.deathObj.attackerKarma -= this.helper.getKarma(this.victimID);
            this.deathObj.attacker.ChatMessage(msgs.loseKarma + " (" + this.helper.getKarma(this.victimID) + ")");
        } else if (this.deathObj.karmaOn && this.deathObj.victimKarma < 0) {
            this.deathObj.attackerKarma += this.helper.getKarma(this.victimID);
            this.deathObj.attacker.ChatMessage(msgs.gainKarma + " (" + this.helper.getKarma(this.victimID) + ")");
        }
        TitlesData.PlayerData[this.attackerID].Kills += 1;
        TitlesData.PlayerData[this.victimID].Deaths += 1;
        if (this.Config.Settings.deathMsgs) this.buildDeathMsg([this.attacker.displayName, this.victim.displayName], [this.attackerID, this.victimID]);
        //this.setRankTitle(killerID, killer); //This needs replaced with a setter
        this.updateKDR([this.attackerID, this.victimID]);
        return false;
    },

    handleSuicide: function() {
        TitlesData.PlayerData[deathObj.victimID].Deaths += 1;
        this.updateKDR([this.deathObj.victimID]);
    },
...
chatHandler.prototype = {
    init: function(arg) {
        this.Chat = {
            ...
            color: this.helper.getColor(steamID),
            msg: arg.GetString(0, "text")
        }
        if (this.Chat.msg.substring(1, 1) === "/" || this.Chat.msg === "") return null;
        if (TitlesData.PlayerData[this.Chat.steamID] === undefined) {
            this.dataHanlder.checkPlayerData(this.Chat.player, this.Chat.steamID);
        }

        var formattedMsg = this.buildChat();
        this.sendChat(formattedMsg)
        return false;
    },
    buildChat: function() {
        if (this.Chat.colorOn && permission.UserHasPermission(this.Chat.steamID, "isStaff")) {
            this.Chat.useColor = "<color=" + this.Config.Settings.staffchatNameColor + ">";
        }

        if (this.Chat.colorOn && !this.Config.Settings.dropRank) {
            this.Chat.titleColor = "<color=" + this.Chat.color[0] + ">[";
        }

        if (this.Config.Settings.useBoth && TitlesData.PlayerData[steamID].Prefix !== "") {
            this.Chat.usePrefix = TitlesData.PlayerData[steamID].Prefix + "]</color> ";
            this.Chat.prefixColor = "<color=" + this.Chat.color[1] + ">[";
        }

        if (!TitlesData.PlayerData[this.Chat.steamID].hidden && !this.Config.Settings.dropRank) this.Chat.useTitle = TitlesData.PlayerData[steamID].Title + "]</color>: ";
        var formattedMsg = this.Chat.prefixColor + this.Chat.usePrefix + this.Chat.useColor + this.Chat.player.displayName + "</color> " + this.Chat.titleColor + this.Chat.useTitle + this.Chat.chatColor + this.Chat.msg + "</color>";
        return formattedMsg;
    },
    sendChat: function(msg) {
        global.ConsoleSystem.Broadcast("chat.add", this.Chat.steamID, msg);
        print(player.displayName + ": " + this.Chat.msg);
        return false;
    }
}
...
commandHandler.prototype = {
    ...
    giveTitle: function(player, cmd, args) {
        ...
        if (prefixHandler && args[2].length && args[3].length) {
            prefixHandler.handleMultiPrefix(getPlayer[1], args[2], args[3]);
            getPlayer[0].ChatMessage(msgs.setSuccs);
        } else {
    ...
    rtHelp: function(player, cmd, args) {
        ...
        if (authLvl >= 2 || this.hasPermission(player, this.Config.Permissions.staff)) {
    ...
    cBuild: function(args) {
        ...
        if (choice.toLowerCase() === "rank") {
            ...
            choice = readline();
            if (choice.toLowerCase() === "yes") {
                this.Config.prefixTitles.push(cRank);
            } else {
                this.cBuild();
            }
        } else {
    ...
    cDelete: function(args) {
        ...
        for (var i = 0; i < this.Config.main.length; i++) {
            if (this.Config.main[i].title.toLowerCase() === targetRank.toLowerCase()) {
                delete this.Config.main[i];
                break;
            } else if (i >= this.Config.main.length && this.Config.main[i].title.toLowerCase() !== targetRank.toLowerCase()) {
...
function RanksAndTitles(author, title, version) {
    ...
    this.prefix = this.Config.Messages;
...
var ranksPrototype = new RanksAndTitles("KillParadise", "Ranks And Titles", V(2, 0, 0));
