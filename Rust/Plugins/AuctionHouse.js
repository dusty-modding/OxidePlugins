var AuctionHouse = {
    ...
    OnPlayerDisconnected: function(player) {
      // start auction data save
      this.offlineHandler(player);
    },
    ...
    offlineHandler: function(user) {
      var steamID = rust.UserIDFromPlayer(user);
    ...
    cmdSwitch: function(player, cmd, args) {
        ...
        switch (args[0]) {
            case "bid":
                if (this.hasPermission(player, perms.Bid)) {
                ...
                break;
            case "buy":
                if (this.hasPermission(player, perms.Buy)) {
            ...
            case "create":
                ...
                break;
            case "list":
                if (this.hasPermission(player, perms.List)) {
    ...
    createAuction: function(player, cmd, args) {
        ...
        if (this.Config.Settings.maxAuctions >= numOfAuctions) {
            ...
            AuctionData.OpenAuctions[numOfAuctions].steamID = steamID;
