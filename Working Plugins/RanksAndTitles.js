var RanksAndTitles = {
        ...
        buildGroups: function() {
            var i = 0,
                mainLen = this.Config.Main.length;
            if (mainLen > 0) {
                for (i; i < mainLen; i++) {
                    if (!GroupExists(this.Config.Main[i].title)) {
                    	print("Building Group... " + this.Config.Main[i].rankTitle);
                        CreateGroup(this.Config.Main[i].title, this.Config.Main[i].rankTitle, this.Config.Main[i].rank);
                    } else {
                        continue;
                    }
                }
            } else if (mainLen <= 0) {
                print("ERROR: Config MAIN is Empty.");
                print("Re Running Default");
                this.LoadDefaultConfig();
            }
        },
        ...
        registerPermissions: function() {
        	var i = 0, j = this.Config.Main.length;
        	for (i; i < j; i++) {
        		if (!permission.PermissionExists(this.Config.Main[j].title)){
        		permission.RegisterPermission(this.Config.Main[j].title, this.Object);
        	}
        }
    },
	...
	------------------------------------------------------------------*/
	 ...
	 ------------------------------------------------------------------*/
        ...
        setRankTitle: function(playerID, player) {
            ...
            var j = this.Config.Main.length,
                s = this.Config.Saved.length
                ...
                for (i; i < j; i++) {
                    for (ii; ii < s; ii++) {
                        if (karmaOn && this.getClosest([], karma) === this.Config.Main[i].karma) {
                            TitlesData.PlayerData[playerID].Title = this.Config.Main[i].rankTitle;
                    	...
                    	if (TitlesData.PlayerData[playerID].plyrGroup !== this.Config.Saved[ii]) {
                    		TitlesData.PlayerData[playerID].plyrGroup = this.Config.Main[i].title;
                    		this.AddUserGroup(playerID, this.Config.Main[i].title);
                    	}
                    }
                }
            }
	...
	------------------------------------------------------------------*/
	   ...
	   ------------------------------------------------------------------*/
        ...
        //Now the function takes care of the issue automatically for the admin, creating data.
        //It then assigns ids, makes sure everyone involved was players,
	...
	------------------------------------------------------------------*/
        ...
        removeTitle: function(player, cmd, args) {
            try {
                var getPlayer = this.findPlayerByName(player, args);
                this.RemoveUserGroup(getPlayer[1], TitlesData.PlayerData[getPlayer[1]].plyrGroup);
                TitlesData.PlayerData[getPlayer[1]].plyrGroup = "";
        ...
        giveTitle: function(player, cmd, args) {
            ...
            if (args[2].length) {
                for (i; i < j; i++) {
                    if (args[2].toLowerCase() === this.Config.Main[i].title.toLowerCase()) {
                        TitlesData.PlayerData[getPlayer[1]].Title = this.Config.Main[i].rankTitle;
                        TitlesData.PlayerData[getPlayer[1]].plyrGroup = this.Config.Main[i].title;
                        this.AddUserGroup(this.Config.Main[i].title);
                    }
                }
        ...
        getColor: function(steamID) {
            ...
            for (var i = 0; i < this.Config.Main.length; i++) {
                if (TitlesData.PlayerData[steamID].Title === this.Config.Main[i].title) {
                    color = this.Config.Main[i].Color;
                    return color;
                }
            }
        }
        ...
        try {
            if (!chatHandler) {
                var colorOn = this.Config.Settings.colorSupport,
                    ...
                    chatColor = this.Config.Settings.chatNameColor,
                    staffChatColor = this.Config.Settings.staffChatNameColor,
                    ...
                    title = TitlesData.PlayerData[steamID].Title,
                    hidden = TitlesData.PlayerData[steamID].hidden;
                var displayName = player.displayName;
                if (this.Config.Settings.usePrefix) {
                    plyrGroup = TitlesData.PlayerData[steamID].plyrGroup
                } else {
                    plyrGroup = null;
                }
                if (msg.substring(1, 1) === "/" || msg === "") return false;
                if (hidden || title === "") {
                    global.ConsoleSystem.Broadcast("chat.add", steamID, "<color=" + chatColor + ">" + displayName + "</color> " + msg);
                    print(player.displayName + ": " + msg);
                    return false;
                } else if (colorOn && authLevel === 0) {
                    global.ConsoleSystem.Broadcast("chat.add", steamID, "<color=" + color + ">[" + plyrGroup + "]</color> " + "<color=" + chatColor + ">" + displayName + "</color>" + " <color=" + color + ">[" + title + "]</color> " + msg);
                    print(player.displayName + ": " + msg);
                    return false;
                } else if (!colorOn && authLevel === 0) {
                    global.ConsoleSystem.Broadcast("chat.add", steamID, "<color=" + chatColor + ">" + "[" + plyrGroup + "] " + displayName + " [" + title + "] " + "</color>" + msg);
                    print(player.displayName + ": " + msg);
                    return false;
                } else if (authLevel >= 1 && colorOn) {
                    global.ConsoleSystem.Broadcast("chat.add", steamID, "<color=" + color + ">[" + plyrGroup + "]</color> " + "<color=" + staffChatColor + ">" + displayName + "</color>" + " <color=" + color + ">[" + title + "]</color> " + msg);
                    print(player.displayName + ": " + msg);
                    return false;
                } else if (authLevel >= 1 && !colorOn) {
                    global.ConsoleSystem.Broadcast("chat.add", steamID, "<color=" + staffChatColor + ">" + "[" + plyrGroup + "] " + displayName + " [" + title + "] " + "</color> " + msg);
                    print(player.displayName + ": " + msg);
                    return false;
                }
            } else {
    ...
    //                 print("Number of Main: " + this.Config.Main.length);
