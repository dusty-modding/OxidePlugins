var RanksAndTitles = {
    ...
    debug: function(player, cmd, args) {
        try {
            if (!this.Config.Setting.deBugOff) {
                var steamID = rust.UserIDFromPlayer(player);
    ...
    checkPlayerData: function(player, steamID) {
        ...
        try {
            if (GroupsAPI) GroupData = this.grabGroupData();
    ...
    //Function to update our groups data called by our very active functions.
    grabGroupData: function() {
        GroupData = data.GetData("Groups");
        GroupData = GroupData || {};
        return GroupData;
    },
    ...
    setRankTitle: function(playerID, player) {
        try {
            ...
            for (i; i < j; i++) {
                ...
                if (TitlesData.PlayerData[playerID].Title !== this.Config.Ranks[i].title) {
                    return this.setCustomTitle(playerId, player);
                }

                if (!GroupsAPI && (TitlesData.PlayerData[playerID].Title === this.Config.Ranks[i].title || TitlesData.PlayerData[playerID].Title === "")) {
                    if (karmaOn && (karma === this.Config.Ranks[i].karma || TitlesData.PlayerData[playerID].Title === this.Config.Ranks[i].title)) {
                    ...
                    } else if (!karmaOn && (kills === this.Config.Ranks[i].killsNeeded || TitlesData.PlayerData[playerID].Title === this.Config.Ranks[i].title)) {
                        player.displayName = TitlesData.PlayerData[playerID].RealName + " [" + this.Config.Ranks[i].title + "]";
                        TitlesData.PlayerData[playerID].Title = this.Config.Ranks[i].title;
                        TitlesData.PlayerData[playerID].Rank = this.Config.Ranks[i].rank;
                    }
            ...
            if (colorOn) {
                getColor = this.Config.Ranks[i].Color;
            }
            if (!useTitles) {
    ...
    setTitle: function(playerID, player) {
        try {
            ...
            for (i; i < j; i++) {
                ...
                if (colorOn) {
                    getColor = this.Config.Titles[i].Color;
                }
    ...
    setCustomTitle: function(playerID, player) {
        try {
            ...
            for (i; i < j; i++) {
                if (!GroupsAPI && (kills === this.Config.Ranks[i].killsNeeded || karma === this.Config.Ranks[i].karma)) {
                    var customOn = true;
                    player.displayName = TitlesData.PlayerData[playerID].RealName + " [" + TitlesData.PlayerData[playerID].Title + "]";
                    TitlesData.PlayerData[playerID].Rank = this.Config.Ranks[i].rank;
                } else if (GroupsAPI && (kills === this.Config.Ranks[i].killsNeeded || karma === this.Config.Ranks[i].karma)) {
                    var customOn = true;
    ...
    OnEntityDeath: function(entity, hitinfo) {
        try {
            ...
            print(e.message.toString());
            if (e.message.toString().search("undefined"))
        }
    },
    ...
    giveTitle: function(player, cmd, args) {
        if (GroupsAPI) {
            var GroupData = this.grabGroupData();
        }
    ...
    OnPlayerChat: function(arg) {
        try {
            ...
            if (this.Config.Setting.colorSupport) {
                var color = this.getColor(steamID);
                global.ConsoleSystem.Broadcast("chat.add", steamID, realName + " <color=" + color + ">[" + title + "]</color> " + msg);
                return false;
            } else {
                global.ConsoleSystem.Broadcast("chat.add", steamID, realName + " [" + title + "] " + msg);
                return false;
            }
        } catch (e) {
            print(e.message.toString());
